...
def make_signal(start_time, end_time, values):
    ...
    for n in range(start_time, end_time+1):
        signal.set_value_at_time(n, values[n - start_time])
...
def signal_from_samples(start_time, end_time, samples):
    ...
    for n, x_n in samples.items():
        signal.set_value_at_time(n, x_n)
...
def max_absolute_difference(first_signal, second_signal):
    diff_signal = first_signal.add(second_signal.multiply(-1))
    
    nonzeros = diff_signal.nonzero_samples()
    
    return max((abs(diff) for _,diff in nonzeros), default=0.0)
    
    # if len(nonzeros) == 0:
    #     return 0.0
    
    # ans = 0.0
    # for _,diff in nonzeros:
