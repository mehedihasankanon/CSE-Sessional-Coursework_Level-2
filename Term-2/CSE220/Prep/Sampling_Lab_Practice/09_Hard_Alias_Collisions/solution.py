...
def collapse_tones(freqs, amps, fs):
    # Merge alias collisions, remove zero totals, and return sorted arrays.
    groups = {}
    for f, amplitude in zip(freqs, amps):
        alias = given_alias(f, fs)
        groups[alias] = groups.get(alias, 0) + int(amplitude)
    kept = sorted(f for f, amplitude in groups.items() if amplitude != 0)
    return np.array(kept, dtype=int), np.array([groups[f] for f in kept], dtype=int)

def evaluate_tones(t, freqs, amps):
    # Evaluate the multitone cosine model, including an empty model.
    result = np.zeros_like(t, dtype=float)
    for f, amplitude in zip(freqs, amps):
        result += amplitude * np.cos(2 * np.pi * f * t)
    return result
